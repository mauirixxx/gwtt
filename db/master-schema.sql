...
DROP TABLE IF EXISTS weapon_attribute_map; DROP TABLE IF EXISTS listinsignias; DROP TABLE IF EXISTS listrunes; DROP TABLE IF EXISTS listruneprofessions; DROP TABLE IF EXISTS listattribute;
...
CREATE TABLE listinsignias (insigniaid TINYINT UNSIGNED NOT NULL AUTO_INCREMENT,professionid TINYINT UNSIGNED NOT NULL,insignia VARCHAR(60) NOT NULL,PRIMARY KEY(insigniaid),UNIQUE KEY uk_insignia(insignia),KEY idx_insignia_profession(professionid),CONSTRAINT fk_insignia_profession FOREIGN KEY(professionid) REFERENCES listruneprofessions(runeprofid) ON DELETE RESTRICT ON UPDATE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE weapon_attribute_map (weaponid TINYINT UNSIGNED NOT NULL,weapattrid TINYINT UNSIGNED NOT NULL,PRIMARY KEY(weaponid,weapattrid),KEY idx_weapon_attribute_attr(weapattrid),CONSTRAINT fk_weapon_attribute_type FOREIGN KEY(weaponid) REFERENCES listtype(weaponid) ON DELETE CASCADE ON UPDATE CASCADE,CONSTRAINT fk_weapon_attribute_attribute FOREIGN KEY(weapattrid) REFERENCES listattribute(weapattrid) ON DELETE CASCADE ON UPDATE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
INSERT INTO weapon_attribute_map(weaponid,weapattrid)
SELECT t.weaponid,a.weapattrid FROM listtype t JOIN listattribute a ON
(t.weapontype='Axe' AND a.weaponattribute='Axe Mastery') OR (t.weapontype='Hammer' AND a.weaponattribute='Hammer Mastery') OR (t.weapontype='Sword' AND a.weaponattribute='Swordsmanship') OR (t.weapontype='Dagger' AND a.weaponattribute='Dagger Mastery') OR (t.weapontype='Scythe' AND a.weaponattribute='Scythe Mastery') OR (t.weapontype LIKE 'Bow (%)' AND a.weaponattribute='Marksmanship') OR (t.weapontype='Spear' AND a.weaponattribute='Spear Mastery') OR
(t.weapontype='Shield' AND a.weaponattribute IN ('Strength','Tactics','Leadership','Command','Motivation')) OR
(t.weapontype IN ('Focus','Wand','Staff') AND a.weaponattribute IN ('Divine Favor','Healing Prayers','Protection Prayers','Smiting Prayers','Soul Reaping','Blood Magic','Curses','Death Magic','Fast Casting','Inspiration Magic','Domination Magic','Illusion Magic','Energy Storage','Fire Magic','Water Magic','Air Magic','Earth Magic','Spawning Power','Channeling Magic','Communing','Restoration Magic'));
...
CREATE TABLE `history` (
  ...
  `drop_type` TINYINT UNSIGNED NOT NULL COMMENT '1=Weapon, 2=Material, 3=Rune/Insignia, 4=Nothing',
  ...
  CONSTRAINT `fk_history_insignia` FOREIGN KEY (`insignia`) REFERENCES `listinsignias` (`insigniaid`) ON DELETE RESTRICT ON UPDATE CASCADE,
