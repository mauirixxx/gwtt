...
on <input name="treasuredate" type="date" value="<?php echo date('Y-m-d'); ?>" required> a <select name="runerarity"><option value="2">Blue</option><option value="3">Purple</option><option value="4">Gold</option></select> armor salvage item with
Rune: <select name="rune"><option value="0">None</option><?php options($con,'SELECT runeid,runes FROM listrunes ORDER BY runeclassid,runeid','runeid','runes'); ?></select>
Insignia: <select name="insignia"><option value="0">None</option><?php options($con,'SELECT insigniaid,insignia FROM listinsignias ORDER BY professionid,insigniaid','insigniaid','insignia'); ?></select>
and <input type="number" name="droppedgold" min="0" max="9999" value="0" required> gold pieces.
...
<?php elseif($locid): ?><div style="text-align:center"><form method="POST"><?php echo gw_csrf_input(); ?><select name="gwdrop" onchange="this.form.submit()"><option selected disabled>Choose drop type</option><option value="1">Weapon</option><option value="2">Rare Material</option><option value="3">Rune / Insignia</option><option value="4">Nothing!</option></select><input type="hidden" name="locationid" value="<?php echo $locid; ?>"><noscript><input type="submit" value="Submit"></noscript></form></div><?php endif; ?>
